...
public struct ImageManager {
  ...
  public static func maxMemory(cost: UInt) {
    ImageCache.default.maxMemoryCost = cost
  }
    ...
    DispatchQueue.global(qos: .utility).async {
      if !ImageCache.default.imageCachedType(forKey: urlStr).cached {
        if let url = URL(string: urlStr) {
          ImageDownloader.default.downloadImage(with: url, options: [], progressBlock: nil, completionHandler: { (img, _, _, data) in
            /// If the downloaded file is gif then should process before storing in local
            if (urlStr as NSString).pathExtension == "gif", let data = data, let gif = DefaultImageProcessor.default.process(item: .data(data), options: []) {
              KingfisherManager.shared.cache.store(gif, original: data, forKey: urlStr)
            } else if let img = img {
  ...
  public static func retrieveCachedImage(url: String?) -> UIImage? {
    guard let url = url else { return nil }
    if let img = ImageCache.default.retrieveImageInMemoryCache(forKey: url) {
      return img
    }
    if let img = ImageCache.default.retrieveImageInDiskCache(forKey: url) {
      return img
    }
    return nil
  }
  ...
  public static func retrieveImages(urls: [String], completed: @escaping([UIImage]) -> Void, needDownload: Action? = nil) {
    DispatchQueue.main.async {
      var result = [UIImage]()
      urls.forEach({ url in
        ImageCache.default.retrieveImage(forKey: url, options: nil) { (img, _) in
          if let img = img {
            result.append(img)
  ...
  public static func renderImage(imgView: UIImageView, urlString: String, blur: Bool = false, effect: Bool = true, placeholder: UIImage? = nil, completionHandler: ((_ image: UIImage?) -> Void)?=nil) {
  ...
  public static func renderImageWithBlur(imgView: UIImageView, urlString: String, downloading: Action? = nil, downloaded: Action? = nil) {
    if let url = URL(string: urlString) {
      DispatchQueue.global(qos: .utility).async {
        ImageCache.default.retrieveImage(forKey: urlString, options: nil, completionHandler: { (img, type) in
          if let img = img {
            DispatchQueue.main.async {
              imgView.image = img
              downloaded?()
            ...
            imgView.kf.setImage(with: url, placeholder: #imageLiteral(resourceName: "icn-defaultAvatar"), options: options, completionHandler: { (img, _, _, _) in
              downloaded?()
            })
          }
        })
          ...
          if let img = img {
            ...
            if ImageManager.isGif(origin) {
              ...
              if let small = URL(string: photos.first ?? "") {
                ...
                imgView.kf.setImage(with: small, placeholder: #imageLiteral(resourceName: "icn-defaultAvatar"), options: coptions, completionHandler: { (_, _, _, _) in
